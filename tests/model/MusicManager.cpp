...
TEST(MusicManagerTest, LoadFilesByArtist) {
    std::filesystem::path file = "../tests/res/MusicTests";
    if (!std::filesystem::exists(file)) {
        FAIL() << "Folder does not exist";
    }

    const std::string artistName = "AnotherArtist";
    ...
    Model::MusicManager musicManager;
    musicManager.collectFilesFromArtist(file, artistName);
...
TEST(MusicManagerTest, LoadFilesByArtistShouldThrow) {
    std::filesystem::path file = "../tests/res/MusicTests";
    if (!std::filesystem::exists(file)) {
        FAIL() << "Folder does not exist";
    }

    const std::string artistName = "AnotherArtistfewjifwejiof";
